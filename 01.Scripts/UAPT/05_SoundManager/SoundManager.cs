...
public class SoundManager : MonoSingleton<SoundManager>
{
    ...
    {
        MixerNullChake();
        _mixer.SetFloat("Music", Mathf.Log10(volume) * 20); PlayerPrefs.SetFloat("BGMVolume", volume);
    }
    ...
    public float GetVolume(SoundType type) => PlayerPrefs.GetFloat($"{type.ToString()}Volume", 0.5f);
    ...
    public static void PlaySound(AudioClip clip, SoundType type)
    {
        SoundPlayer sound = PoolManager.SpawnFromPool($"{type}Obj", Vector3.zero).GetComponent<SoundPlayer>();
